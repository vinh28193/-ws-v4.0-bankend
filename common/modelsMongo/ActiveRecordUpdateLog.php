...
class ActiveRecordUpdateLog extends \yii\mongodb\ActiveRecord
{
    ...
    public static function register($action, ActiveRecord $model, $userId = null)
    {
        if ($userId === null) {
            $userId = Yii::$app->getUser()->getId();
        }
        $model->on(ActiveRecord::EVENT_AFTER_UPDATE, function ($event) use ($action, $userId) {
            ...
            if (!empty($dirtyAttribute)) {
                ...
                $classShort = $reflection->getShortName();
                if ($classShort === 'Order') {
                    $pk = $sender->ordercode;
                }
                ...
                if ($action === 'beforeConfirm' && ($original = self::findOriginal($classShort, $pk)) !== null) {
                ...
                $newLog->object_class = $classShort;
                ...
                if ($reflection->getShortName() === 'Order') {
                    ...
                    foreach ($dirtyAttribute as $name => $val) {
                        $oldValue = $oldAttribute[$name];

                        $alias = $sender->getAttributeAlias($name);
                        $isAmount = self::getTypeName($alias, 'amount');
                        if (in_array($name, $sender->timestampFields()) || WeshopHelper::compareValue($oldValue, $val, $isAmount ? 'float' : 'integer')) {
                            continue;
                        }

                        if (($currency = self::getCurrency($alias)) !== false) {
                            $val = self::getStoreManager()->showMoney($val, $currency);
                            $oldValue = self::getStoreManager()->showMoney($oldValue, $currency);
                        }
                        $info[] = "<span class='font-weight-bold'>- {$sender->getAttributeAlias($name)} :</span> <br> Changed from `{$oldValue}` to `$val`";
                    }
    ...
    protected static function findOriginal($objectClass, $objectIdentity)
    {
        return self::find()->where(['and', ['status' => 'active'], ['type' => 'original'], ['object_class' => $objectClass], ['object_identity' => $objectIdentity]])->one();
    }

    public static function getCurrency($value)
    {
        $value = strtolower($value);
        if (WeshopHelper::isSubText($value, 'local')) {
            return self::getStoreManager()->getCurrencyName();
        } else if (WeshopHelper::isSubText($value, 'amount')) {
            return self::getStoreManager()->moneyShowWith === StoreManager::MONEY_SHOW_MODE_CURRENCY ? 'USD' : '$';
        }
        return false;
    }

    public static function getTypeName($amount, $type = 'local')
    {
        return WeshopHelper::isSubText($amount, $type);
    }
