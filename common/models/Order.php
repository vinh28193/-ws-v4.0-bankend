...
class Order extends DbOrder implements RuleOwnerAccessInterface
{
    ...
    const STATUS_CONTACTING = 'CONTACTING'; // đã chăm sóc đơn hàng xong.
    const STATUS_AWAITING_PAYMENT = 'AWAITING_PAYMENT'; // confirm chờ thanh toán
    ...
    const STATUS_AWAITING_CONFIRM_PURCHASE = 'AWAITING_CONFIRM_PURCHASE';
...
//            ],
    ...
    {
        return $this->hasOne(PaymentTransaction::className(), ['order_code' => 'ordercode']);
    }
...
//    }
    ...
    {
        $this->buyer_name = Html::encode($this->buyer_name);
        $this->buyer_address = Html::encode($this->buyer_address);
        $this->note_by_customer = Html::encode($this->note_by_customer);
        $this->receiver_name = Html::encode($this->receiver_name);
        $this->receiver_address = Html::encode($this->receiver_address);
