...
use common\modelsMongo\BoxmeLogSendReturnShipmentWs ;
use common\models\Order;
use Yii;
use yii\web\NotFoundHttpException;
use yii\web\ServerErrorHttpException;

class RestBmLogSendReturnShipmentWsController extends BaseApiController
{
    /** Role :
        case 'cms':
        case 'warehouse':
    ...
    **/
    ...
    {
        $response = BoxmeLogSendReturnShipmentWs::search($params = '');
    ...
    public function actionCreate()
    {
        ...
        if (isset($_post) !== null) {
            $model = new BoxmeLogSendReturnShipmentWs();
            ...
            $_rest_data = ["BoxmeLogSendReturnShipmentWs" => [
                ...
                "LogTypeSendShipmentReturn" =>  $_post['LogTypeSendShipmentReturn'], // "Order hoăc Product", // LogType : Order | Product : and Id để join
    ...
    {
        if ($id !== null) {
            $response = BoxmeLogSendReturnShipmentWs::find()
                ->where(['id' => $id])
                ->asArray()->all();
    ...
    {
        if (($model = BoxmeLogSendReturnShipmentWs::findOne($id)) !== null) {
            return $model;
        } else {
            Yii::$app->api->sendFailedResponse("Invalid Record requested");
        }
