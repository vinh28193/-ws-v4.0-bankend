...
#   `is_discontinued` bit NOT NULL DEFAULT 0 ,
  ...
  `units_in_on_order`,
  `product_available`,
  `discount_available`decimal(12,2) NOT NULL,
  `current_order`,
  `picture`,
  `ranking`,
  `note`,
  ...
  KEY `FK_Product_category_id_Category` (`category_id`),
  CONSTRAINT `FK_Product_category_id_Category` FOREIGN KEY `FK_Product_category_id_Category` (`category_id`) REFERENCES `Category` (`category_id`)
...
(
  ...
  CONSTRAINT `FK_OrderDetailDelivery_order_item_id_OrderItem` FOREIGN KEY `FK_OrderDetailDelivery_order_item_id_OrderItem` (`order_item_id`) REFERENCES `OrderItem` (`order_item_id`),
